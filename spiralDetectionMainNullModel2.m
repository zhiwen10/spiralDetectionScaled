...
axis image;
hold on;
rectangle('Position',[251,201,250,250],'EdgeColor','w');
subplot(1,2,2); 
imagesc(mimg(201:450,251:500));
axis image;
%%
mimg = readNPY(fullfile(serverRoot, 'blue','meanImage.npy'));
mimg = mimg(201:450,251:500);
U2 = U(201:450,251:500,:);
...
rpAlli = [];
rpAll = [];
rp = randperm(size(U1,1)*size(U1,2)); 
...
for kk=1:7
    ...
    t1 = t(frameTemp);    
    [trace2d1,traceAmp1,tracePhase1] = spiralPhaseMapNullModel5(U1,dV1,t1,params,rate,rp);
...
%% sanity check
frame2 = 36:135; framei = 75;
rate1 = 1;
spiralSanityCheckNullModel;
%% kernel density plot
figure; 
ax2 = subplot(1,1,1);
% imagesc(mimg);
...
%%
frameID2 = 6804:6819;
frameN = frameStart:frameEnd;
istart = find(frameN==frameID2(1));
iend = find(frameN==frameID2(end));
%%
tracePhase = tracePhase1(:,:,istart:iend);
trace2d1 = trace2d1(:,:,1+35:end-35);
trace2d2 = trace2d1(:,:,istart:iend);
%%
pwAllEpoch = pwAll(ismember(pwAll(:,5),frameID2),:);
pwAllEpoch(:,5) = pwAllEpoch(:,5)-frameID2(1)+1;
frameID = 1:numel(frameID2);
rpadding = 20;
[cells] = getCellMain(tracePhase1,pwAllEpoch,frameID,rpadding);
%% epoch spirals
